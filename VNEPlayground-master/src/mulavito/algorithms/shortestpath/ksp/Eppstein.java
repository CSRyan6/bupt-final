...
public class Eppstein<V, E> extends KShortestPathAlgorithm<V, E> {
	...
	/**
	 * Create <math>\delta(e)</math> for each edge in graph {@link #}.
	 * 
	 * @param target
	 *            The target
		...
		for (E e : graph.getEdges()) {
			if (edgeMap.get(e) == null) {
				...
				double l = nev.transform(e).doubleValue() /* l(e) */
						+ lengthMap.get(graph.getEndpoints(e).getFirst()) /* d(head(e), t) */
						- lengthMap.get(graph.getEndpoints(e).getSecond()) /* d(tail(e), t) */;
				edgeMap.put(e, l);
			}
		}
