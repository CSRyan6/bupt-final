...
public class MiscelFunctions {
		...
		for (Node<?> node : net.getVertices()) {
			...
			if (net instanceof SubstrateNetwork)
				for (SubstrateLink sLink : ((SubstrateNetwork) net)
						.getIncidentEdges((SubstrateNode) node)){

					if(((SubstrateNetwork) net)
							.getEndpoints(sLink).getFirst() == node){
						tempNodeNeighbors.add(((SubstrateNetwork) net)
								.getEndpoints(sLink).getSecond());
					}else if(((SubstrateNetwork) net)
							.getEndpoints(sLink).getSecond() == node) {
						tempNodeNeighbors.add(((SubstrateNetwork) net)
								.getEndpoints(sLink).getFirst());
					}
			...
			if (net instanceof VirtualNetwork)
				for (VirtualLink vLink : ((VirtualNetwork) net)
						.getOutEdges((VirtualNode) node)){
					if(((VirtualNetwork) net)
							.getEndpoints(vLink).getFirst() == node){
						tempNodeNeighbors.add(((VirtualNetwork) net)
								.getEndpoints(vLink).getSecond());
					}else if(((VirtualNetwork) net)
							.getEndpoints(vLink).getSecond() == node) {
						tempNodeNeighbors.add(((VirtualNetwork) net)
								.getEndpoints(vLink).getFirst());
					}
				}
	...
	public static void clearVnodeMappings(VirtualNetwork vNet, VirtualNode vNode) {
		...
		for (VirtualLink link : vNet.getIncidentEdges(vNode)) {
