...
public class BFSNRnodeMappingPathSplitting extends AbstractNodeMapping {
	...
	public BFSNRnodeMappingPathSplitting(SubstrateNetwork sNet, double wCpu,
                                         double wBandwidth, double epsilon,
	...
	public BFSNRnodeMappingPathSplitting(
			double wCpu, double wBandwidth, double epsilon,
			int dist, int maxHop) {
		this.epsilon = epsilon;
		this.dist = dist;
		this.backtrackLimit = 0;
	...
	protected boolean nodeMapping(SubstrateNetwork sNet, VirtualNetwork vNet) {
		...
		// ///////////////////////////////////////////////
		// This part is used to set backtrack and do candidate selection
		...
		if (problemSolver.problemFeasible()) {
			...
			for (Iterator<List<String>> cad = solverResult.keySet().iterator(); cad
				...
				if (srcSnode.getId() == Integer.parseInt(tmpValues.get(0))
					...
					// hidden hop mapping (When the substrate node is on the path of link mapping, the node is called hidden hop)
